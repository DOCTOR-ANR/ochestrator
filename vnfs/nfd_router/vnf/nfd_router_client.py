...
class GenericClient(object):
    ...
    def send_request(self,method,action,data=None):
        ...
        try:
            ...
            self.logger.debug("can't send request")
            raise Exception


class RouterClient(GenericClient):
    """ formats and sends requests to configure faces """

    def __init__(self, logger, vnfm_host, vnfm_port=3999 ):
        GenericClient.__init__(self, logger, vnfm_host,vnfm_port,"router")
        self.logger.debug("router client UP")
        ...
        while True:
            try:
                ...
                config = {"container":containerID,
                        ...
                        "listening_port":4999}
                self.send_request('POST', 'notifications/router_UP', config)
                break
            except Exception:
                self.logger.debug("waiting for vnfm ...")
                time.sleep(1)
